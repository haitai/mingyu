...
test('DivinationPanel 的 getSummaryBlocks 已抽到 lib/divination/summary,UI 组件不再嵌套数据格式化', async () => {
  ...
  assert.match(liuren.lines.join('\n'), /古籍依据：《大六壬大全》九宗门取传法之知一\/比用/);
  assert.match(liuren.lines.join('\n'), /四课关系：/);
  assert.match(liuren.lines.join('\n'), /三传主线：初传子 → 中传丑 → 末传寅/);
  assert.match(liuren.lines.join('\n'), /课体：龙德卦、连珠卦/);
  assert.match(liuren.lines.join('\n'), /神煞：旬奇临初传；天马并发/);
  assert.doesNotMatch(liuren.lines.join('\n'), /地盘：子、丑、寅/);
  assert.doesNotMatch(liuren.lines.join('\n'), /天盘：/);
...
test('大六壬起课结果已使用盘位样式展示天地盘四课三传', async () => {
  const { readFileSync } = await import('node:fs');
  const { resolve } = await import('node:path');

  const resultSource = readFileSync(
    resolve('src/components/DivinationPanel/DivinationResult.tsx'),
    'utf8',
  );
  const styleSource = readFileSync(resolve('src/styles.css'), 'utf8');

  assert.match(resultSource, /function LiurenBoard/);
  assert.match(resultSource, /function LiurenPlateGrid/);
  assert.match(resultSource, /liuren-plate-grid/);
  assert.match(resultSource, /liuren-lessons-grid/);
  assert.match(resultSource, /liuren-transmission-chain/);
  assert.match(resultSource, /session\.method === 'liuren'/);
  assert.match(resultSource, /const isLiurenResult = session\.method === 'liuren'/);
  assert.match(resultSource, /!\{?isLiurenResult\}?/);
  assert.match(resultSource, /!isLiurenResult \? \(\s*<>\s*<div className="divination-tag-cloud">/);
  assert.match(styleSource, /\.liuren-plate-grid/);
  assert.match(styleSource, /\.liuren-plate-center/);
  assert.match(styleSource, /\.liuren-transmission-card:not\(:last-child\)::after/);
});
