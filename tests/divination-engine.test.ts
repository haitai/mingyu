...
test('占卜引擎会在当前项目本地统一构建提示词骨架', () => {
  ...
  assert.match(source, /'【排盘信息】'/);
  assert.match(source, /'【分析对象】'/);
  assert.match(source, /'【解读范围】'/);
...
test('占卜提示词改为角色加信息加问题的标准架构，并精简重复要求', () => {
  ...
  assert.match(source, /不得编造已提供资料没有给出的卦象细节、盘局数据、牌位信息或签文条件/);
  assert.match(source, /允许基于已提供资料做本体系推理/);
...
test('大六壬提示词会引入可选分析思路', () => {
  ...
  assert.match(source, /分析类型：/);
  assert.match(source, /回答格式：/);
});
