---
name: aov-mingyu-api
description: 通过 aov.cc 公开 API 调用命理和占卜能力。用于需要八字排盘、紫微斗数排盘、六爻、梅花易数、奇门遁甲、大六壬、塔罗、三山国王灵签，或基于排盘结果生成 AI 解读提示词的任务。
---
...
2. 排盘类任务直接调用对应 `POST` 接口。
3. 需要 AI 解读时，先调用排盘接口，再把排盘结果作为 `data` 传给 `POST /divination/prompt`。
...
- `POST /divination/prompt`：基于排盘结果生成结构化 AI 解读提示词。
